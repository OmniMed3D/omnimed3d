...
const TOOLTIP_TEXT: Record<string, string> = {
  ...
  "low-memory-mode-enabled":
    "Reduces GPU memory usage (~266MB on the demo CT) at a small shading-cost tradeoff -- overrides automatic device-memory detection. Applies the next time a volume is loaded, not to one already on screen.",
  "reload-volume":
    "Reloads the current volume so Low-Memory Mode/Downsample Factor changes take effect -- both only apply on load, not instantly. Disabled until a volume has been loaded.",
  ...
  "background-preset-gray":
    "Mid-gray background -- reduces eye strain for extended reading, a common radiology-viewer convention.",
  ...
  "stat-gpu-pass":
    "Actual GPU time spent rendering (WebGPU timestamp-query), not wall-clock frame time -- unlike Perf above, this isn't capped by the display's refresh rate, so it stays meaningful even when Perf is vsync-limited. Shows \"unsupported\" if this browser/GPU doesn't support GPU timestamp queries.",
  "stat-canvas": "Canvas backing-store resolution, in device pixels (post-devicePixelRatio scaling).",
  "stat-low-memory":
    "Whether this volume was loaded skipping the precomputed gradient texture (saves ~266MB of GPU memory on the demo CT, at a small per-frame shading cost) -- auto-detected from device memory/iOS, or forced via ?lowMemory=1 or ?lowMemory=0.",
  "stat-gpu-vendor": "GPU vendor, as reported by the browser's WebGPU adapter.",
  "stat-gpu-device":
    "GPU device name, as reported by the browser's WebGPU adapter -- often unavailable (n/a) since browsers restrict this for fingerprinting reasons.",
