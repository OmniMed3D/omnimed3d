...
const TOOLTIP_TEXT: Record<string, string> = {
  ...
  "low-memory-mode-enabled": "Reduces GPU memory usage (~266MB on the demo CT) at a small shading-cost tradeoff -- overrides automatic device-memory detection. Applies the next time a volume is loaded, not to one already on screen.",
  ...
  "stat-low-memory": "Whether this volume was loaded skipping the precomputed gradient texture (saves ~266MB of GPU memory on the demo CT, at a small per-frame shading cost) -- auto-detected from device memory/iOS, or forced via ?lowMemory=1 or ?lowMemory=0.",
