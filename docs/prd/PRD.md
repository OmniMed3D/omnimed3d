...
| **Acceptable Accuracy Degradation Threshold** *(Proposed 2026-08-21, pending team confirmation — not yet marked Resolved)* | **Proposal, backed by measured data (see `docs/verification/inference-worker.md` §4): a mean Dice degradation of no more than 1.0 percentage point vs. the FP32 reference, and no individual evaluated slice below 0.98 Dice for either class.** Measured worst case today: INT8 right-lung Dice mean 0.9985 (0.15pp degradation), single-slice minimum 0.9931 (0.69pp) — clears both parts of the proposed threshold with margin; FP16 is effectively at parity (mean 1.0000, minimum 0.9998-0.9999). The two-part form (mean + per-slice floor) is intentional: a mean-only gate could hide one badly-degraded outlier slice averaged out by many near-perfect ones. This is a proposal, not a unilateral AI-track decision (unlike the Upscaling Computation Location row below) — this item's own Decision Owner is the whole team, not just this track, so it stays here for confirmation rather than being marked Resolved. | **Open for team alignment** — proposal above ready for review, not yet confirmed. |
