...
| **Engine Integration Point** | Engine consumes this contract by extending its volume-texture sampling path with a mask channel, avoiding redundant JS-side compositing layers. **MVP scope (revised 2026-08-12):** rather than the brick-atlas/page-table indirection originally described here, the engine lazily allocates an R8Uint full-resolution 3D mask texture and writes each incoming slice directly into its Z-slice region, rejecting any slice whose `volumeId` or dimensions don't match the currently loaded volume — the brick-atlas/LOD system is deferred post-MVP (engine roadmap reprioritized WASM-first; bricking is a later milestone), and this row should be revised back to the brick-based description once that lands. **Updated 2026-08-18:** the Web Application Shell that receives `H` from the Inference Worker and calls into this compositor now exists (`viewer/src/shell/`) and is verified end-to-end in a real browser, including out-of-order slice delivery and stale-`volumeId` rejection (`docs/verification/shell-mask-integration.md`). The remaining gap is not this data path but visual rendering itself: the engine's render pass doesn't yet sample the volume/mask textures it receives (no raymarch/shading pass — see REQ-R02/R03), so nothing appears on screen yet even though the textures are being populated correctly. |
...
| Acceptable Accuracy Degradation Threshold | Team agreement on allowable Dice/IoU reduction margins post-quantization. | Open for team alignment. |
...
| **Upscaling Computation Location** | Whether to perform Nearest-Neighbor upscaling on a WebGPU compute shader or in a WASM loop—see 6.3/10.1. | **AI Track** — to be decided based on empirical measurement during implementation. |
