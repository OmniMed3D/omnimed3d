...
| Git-Tracked Since | 2026-08-16 (first committed snapshot; full decision history in `docs/prd/CHANGELOG.md`) |
| Status | Final (First Submission, 2026-08-27) |
...
| ID | Requirement | Priority |
| --- | --- | --- |
| REQ-A01 | Acquisition of open-source segmentation models and conversion to ONNX format. **Primary target organ: lung; model: lungmask (R231, Apache-2.0).** Training/test dataset: LIDC-IDRI (see 10.2). | P0 |
...
| REQ-C04 | Inter-Worker Data Flow Contract across the 3-Worker Pipeline (Parse → Inference / Render)—defining ownership, transfer mechanisms, and exact timing of Transferable transfers. | P1 | Alignment requirement ensuring worker isolation structures (REQ-A05/A09/A15/A16) do not conflict with the rendering engine's ingestion paths. Joint effort between Engine and AI tracks. Buffer separation (REQ-A15) and the engine-parser-disabled-in-browser-path exclusion (§6.2) follow from this. The Inference Worker → Engine handoff specifically is owned by the Web Application Shell, not either worker directly — see §5.3.2. |

#### 5.3.1 REQ-C01 Mask Data Contract

| Parameter | Value |
| --- | --- |
| Data Type | `uint8` single-channel (class indices); maintained as `uint8` while extending class counts for multi-class support. |
| Coordinate System & Axis Ordering | Identical axis ordering and voxel spacing to the input volume (raw HU tensor, per REQ-A04). No independent resampling. |
| Resolution | Always delivered at the same resolution as the original DICOM volume (the input handed off by the Parse Worker). If the model's input resolution is lower (e.g., inference after downsampling), the mask must be upscaled to the original resolution before crossing this contract boundary. |
| Upscaling Method | Only Nearest-Neighbor interpolation is used. Value-interpolating methods such as Bilinear/Trilinear are prohibited, since they would produce values that don't exist between `uint8` class indices (e.g., 0.3, 1.7). |
| Upscaling Responsibility | Owned by each model adapter's `postprocess` stage (REQ-A17), not the rendering engine. By the time the REQ-C01 boundary is crossed, the mask must already be aligned to the same resolution as the original volume; the rendering engine implements its compositor on the assumption that the volume and mask textures are always aligned 1:1. |
...
| Verification Criteria (Stretch) | Integration of a second model architecture (e.g., 3D network) via separate adapter, demonstrating rendering/pipeline reusability without code modifications to core interfaces. Minimal post-processing (e.g., argmax + upscaling) is sufficient; clinical-grade post-processing polish is not required. |
| Engine Integration Point | The engine consumes this contract by extending its volume-texture sampling path with a mask channel, avoiding redundant JS-side compositing layers. It allocates an R8Uint full-resolution 3D mask texture alongside the volume texture when a volume loads, and writes each incoming slice directly into its Z-slice region, rejecting any slice whose `volumeId` or dimensions don't match the currently loaded volume — allocating the mask texture eagerly (rather than lazily on the first mask slice) means the render pass always has a valid, zero-initialized (all-background) mask texture to bind even before any segmentation slice has arrived, preserving the "volume renders immediately, mask overlay fills in progressively" decoupling model this contract commits to. The render pass itself is a front-to-back raymarch (REQ-R02) with clinical window/level via a transfer-function LUT (REQ-R03) and mask-overlay compositing in the same pass. Verified visually against real DICOM data end-to-end (`viewer/tests/e2e/shell-mask-integration.spec.ts`). The brick-atlas/LOD indirection described in earlier drafts of this section is deferred post-MVP; this row should be revised to the brick-based description once that lands. |
...
| --- | --- | --- |
| DICOM Parsing (Client) | The shared `dicom-parser` C++ library (compiled to WASM), executing in a dedicated **Parse Worker**. A from-scratch parser rather than an existing toolkit port (DCMTK/GDCM/ITK) — see Section 6.2. | Current scope: uncompressed transfer syntaxes only (Explicit/Implicit VR Little Endian). Compressed pixel data (JPEG 2000/JPEG-LS/RLE) is a known limitation, not yet implemented — see Section 10.1. |
...
| **Mask Upscaling Computational Cost** | The lower the model's inference resolution (e.g., 192×192), the larger the upscaling workload to restore it to the original slice resolution (e.g., 512×512). Runs as a WASM loop (Section 10.2), measured at 1-4ms per slice — well within the AI inference latency target in Section 4 (500ms per slice). |
...
| MVP DICOM parser (`dicom-parser/`, shared library) supports only uncompressed transfer syntaxes | Real-world DICOM files using JPEG 2000 / JPEG-LS / RLE compressed pixel data will fail to decode, narrowing which files can be demoed | Explicit MVP scope: the from-scratch shared parser targets Explicit/Implicit VR Little Endian (uncompressed) only — writing JPEG 2000/RLE codecs from scratch was not feasible within this phase. The designated LIDC-IDRI test dataset (Section 10.2) is predominantly distributed uncompressed, so demo/submission risk is low; revisit if a compressed-format need surfaces. |
| Upscaling computation could become a bottleneck on low-spec devices | The larger the gap between model resolution and original volume resolution, the more the postprocess stage's latency grows, potentially threatening the AI inference latency target in Section 4 (500ms per slice). | **Resolved:** measured at 1-4ms per slice via a WASM loop (Section 10.2) — negligible against the >200ms inference time itself. No WebGPU compute shader was needed; revisit only if a future higher-resolution model changes this. |

### 10.2 Key Decisions

Substantive decisions not already fully captured by their own requirement row (§5), kept here as a durable record.

| Item | Decision | Owner |
| --- | --- | --- |
| Training & Evaluation Dataset Choice | LIDC-IDRI (via TCIA), CC BY 3.0, chosen over derived subsets such as LUNA16 (`.mhd/.raw`, not native DICOM) or Medical Segmentation Decathlon Task06_Lung (NIfTI, narrower vendor diversity). Distributed as native DICOM from 7 academic institutions and 8 imaging equipment vendors, spanning 4 scanner manufacturers / 17 CT models — satisfies §9's 3+ distinct vendor/institution success criterion directly. | AI Track |
| Acceptable Accuracy Degradation Threshold | A mean Dice degradation of no more than 1.0 percentage point vs. the FP32 reference, and no individual evaluated slice below 0.98 Dice for either class (backed by measured data, `docs/verification/inference-worker.md` §4). Measured worst case: INT8 right-lung Dice mean 0.9985 (0.15pp degradation), single-slice minimum 0.9931 (0.69pp) — clears both parts of the threshold with margin; FP16 is effectively at parity (mean 1.0000, minimum 0.9998-0.9999). The two-part form (mean + per-slice floor) is intentional: a mean-only gate could hide one badly-degraded outlier slice averaged out by many near-perfect ones. | AI Track |
| Upscaling Computation Location | A WASM loop, not a WebGPU compute shader. Measured directly (`docs/verification/inference-worker.md` §3): postprocess (argmax + Nearest-Neighbor upscale) takes 1-4ms per slice across all three model variants — negligible next to the >200ms this stage's own inference time, and nowhere near threatening the 500ms/slice target (§4). Revisit only if a future higher-resolution model or larger upscale ratio makes this stage non-negligible. | AI Track |
| Second Model / Stretch Goal | Under consideration as MONAI's `spleen_ct_segmentation` (spleen), time permitting — see §5.3.1's stretch verification criteria. Not committed for this submission. | AI Track |
...
| Optional Backend Adapter Priority | Deciding whether REQ-A07/A12 remains P2 or shifts higher based on enterprise use case requirements. | On hold for post-submission review. |
